...
class AgentTaskManager(InMemoryTaskManager):
            ...
            async for item in self.agent.stream(query, task_send_params.sessionId):
                ...
                if not is_task_complete:
                    ...
                    if isinstance(item["content"], dict):
                        ...
                        task_state = TaskState.COMPLETED
                        parts = [TextPart(text=item["content"])]
    ...
    async def _invoke(self, request: SendTaskRequest) -> SendTaskResponse:
        """Non-streaming invoke - uses streaming internally but collects final result"""
        task_send_params: TaskSendParams = request.params
        query = self._get_user_query(task_send_params)
        ...
        try:
            ...
            # Iterate through stream to get final result
            async for item in self.agent.stream(query, task_send_params.sessionId):
                is_task_complete = item["is_task_complete"]
                ...
                if is_task_complete:
                    task_state = TaskState.COMPLETED
                    if isinstance(item["content"], dict):
                        result = json.dumps(item["content"], ensure_ascii=False)
                    else:
                        result = item["content"]
                    break
            
            if not result:
                result = "Không thể xử lý yêu cầu"
