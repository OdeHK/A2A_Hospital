...
class CostAgent:
    ...
    **Nguồn dữ liệu chính và duy nhất để trích xuất danh sách bệnh** trong ngữ cảnh này là phần **Final response_parts** do HostAgent gửi (ví dụ: HostAgent.send_message -> Final response_parts: [...]) và goi_kham_vip_full.json (các gói khám với giá, items).
    ...
    ⚙️ **XỬ LÝ KHI CHỈ CÓ DỮ LIỆU CHI PHÍ HOẶC RAG KHÔNG TRẢ VỀ KẾT QUẢ**:
    - Nếu dữ liệu chỉ có chi phí:
        - Nếu có giá cụ thể → dùng trực tiếp.
        - Nếu không có hoặc trống → bạn **phải sinh ra bảng giá hợp lý** dựa trên triệu chứng bệnh và tên bệnh trong câu trả lời của Symptom Agent hoặc user query, theo bảng mẫu sau:

          - GÓI KHÁM TỔNG QUÁT CƠ BẢN : 4.000.000đ  
          - GÓI KHÁM TỔNG QUÁT NÂNG CAO : 7.000.000đ (Nam)  
          - GÓI KHÁM TỔNG QUÁT CAO CẤP : 17.000.000đ  
          - GÓI KHÁM TẦM SOÁT NGUY CƠ ĐỘT QUỴ : 6.000.000đ  
          - GÓI KHÁM TẦM SOÁT TIM MẠCH : 6.000.000đ  
          - GÓI KHÁM TẦM SOÁT UNG THƯ : 9.500.000đ (Nam) / 9.800.000đ (Nữ)  
          - GÓI KHÁM TẦM SOÁT THẬN NIỆU NAM KHOA : 2.500.000đ  
          - GÓI KHÁM TẦM SOÁT VIÊM GAN : 3.500.000đ  
          - GÓI KHÁM TẦM SOÁT GAN NHIỄM MỠ : 3.500.000đ  
          - GÓI KHÁM TẦM SOÁT BỆNH LÝ ỐNG TIÊU HÓA KHÔNG CAN THIỆP : 2.500.000đ  
          - GÓI KHÁM TẦM SOÁT BỆNH LÝ ỐNG TIÊU HÓA CÓ CAN THIỆP : 3.500.000đ  
          - GÓI KHÁM TẦM SOÁT CƠ XƯƠNG KHỚP : 2.500.000đ  
          - GÓI KHÁM TẦM SOÁT UNG THƯ (Nội soi dạ dày-đại tràng gây mê) : 14.500.000đ  
          - GÓI KHÁM TỔNG QUÁT TIÊU CHUẨN THỨ 7 : 3.500.000đ  
          - GÓI CT-SCAN sọ não không tiêm thuốc cản quang : 1.000.000đ  
          - GÓI CT-SCAN hàm mặt có tiêm thuốc cản quang : 1.950.000đ  

    - Dựa theo nhóm bệnh (tiêu hóa, gan, tim mạch, tiết niệu, xương khớp, tổng quát) để chọn 2–3 gói hợp lý trong danh sách trên.
    - Không được trả về lỗi như “Tôi cần dictionary đầu vào” hay “Không có dữ liệu”.
    ...
    def detect_intent(self, query: str, final_response_parts: list) -> str:
            ...
            symptom_keywords = ["tôi có thể mắc bệnh gì", "gần đây tôi có các triệu chứng", "triệu chứng", "ra máu", "sốt", "mỏi", "mờ mắt", "chóng mặt", "bị", "sưng", "nôn", "đau bụng"]
            cost_keywords = ["gói", "giá", "chi phí", "bao nhiêu", "tốn", "phí", "cost"]

            if any(k in query for k in symptom_keywords) and any(k in query for k in cost_keywords):
                return "symptom+cost"
            if any(k in query for k in symptom_keywords):
                return "symptom"
            if any(k in query for k in cost_keywords):
                return "cost-only"
            return "symptom" if final_response_parts else "unknown"
...
# --- ainvoke: log rõ hơn, normalize kết quả trước khi trả ---
    async def ainvoke(self, input_dict: dict[str, Any]) -> dict[str, Any]:
        ...
        if not final_response_parts and intent in ["symptom+cost", "cost-only"]:
            ...
            logger.debug("[ainvoke] Fallback: Using query as final_response_parts.")
...
# --- stream: xử lý defensive và normalize mọi chunk trước khi yield ---
    async def stream(self, message: HumanMessage) -> AsyncIterable[dict]:
        ...
        if not final_response_parts and intent in ["symptom+cost", "cost-only"]:
            final_response_parts = [query]
            logger.debug("[stream] Fallback: Using query as final_response_parts.")
